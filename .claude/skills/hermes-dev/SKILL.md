...
- **에러 처리**:
  ...
  - 그 외 → 지수 백오프 `2^attempt * 2000ms`. 네트워크/타임아웃 오류는 `status`가 `undefined`라
    이 경로로 떨어진다(정상 동작). 2026-07-11 프로덕션 run에서 `실패 (undefined, 시도 1/3)` 후
    재시도로 회복 확인 — `undefined`를 특별 처리하려 들지 마라.
- **키 로테이션**: `getNextApiKey()`(파이프라인) / `nextGeminiKey()`(agents-slack)가
  `GEMINI_API_KEY`~`GEMINI_API_KEY_5`(최대 5개)를 라운드로빈. 복수 구글 계정 키로 RPM 한도를
  분산한다. 호출마다 다음 키를 쓴다. **주의**: 파이프라인 실행처인 GitHub Actions에선 Secrets에
  키를 넣는 것만으로 부족하다 — `trend-pipeline.yml`의 `env:`에도 `GEMINI_API_KEY_N` 줄을 추가해야
  런타임에 보인다(2026-07-11 기준 `_2`~`_5`는 workflow env 미등록 상태라 사실상 1키로 운영 중).
