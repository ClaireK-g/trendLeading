// Gemini 호출 유틸 — 키 로테이션·모델 폴백·429/503 처리. src/extractor.js와 완전 독립 구현
// (buzz 모듈 격리 원칙). 무료 티어 제약(RPM·오버로드)에 대한 대응 패턴은 동일하게 이식.
  ...
  for (const model of config.llm.models) {
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        if (model !== config.llm.models[0]) console.log(`[buzz:gemini] ${model} 사용`);
        ...
        if (attempt < MAX_RETRIES - 1) {
          ...
          console.warn(`[buzz:gemini] ${model} 실패 (${status}, 시도 ${attempt + 1}/${MAX_RETRIES}), ${delay / 1000}초 후 재시도...`);
          await sleep(delay);
        }
      }
    }
    console.warn(`[buzz:gemini] ${model} 실패, 다음 모델 시도...`);
  }
  console.error('[buzz:gemini] 모든 모델 실패');
  return [];
}
